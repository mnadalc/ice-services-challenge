          ...
          {songs.map((song) => (
            <tr key={song.id} className="border-b border-gray-800 hover:bg-gray-300">
              ...
              <td className="px-4 py-3 text-sm">
                <button
                  onClick={() => issueInvoice(song.author, song.progress, song.id, song.songName)}
                  className="rounded bg-blue-600 px-3 py-1 text-sm font-medium text-white hover:bg-blue-700"
                  aria-label={`Issue invoice for ${song.songName} by ${song.author}`}
                >
                  Issue Invoice
                </button>
                {lastIssuedPerSong.get(song.id) && (
                  <span className="ml-3 text-xs text-gray-600">
                    Last: {formatDate(lastIssuedPerSong.get(song.id)!.issuedAt)} -{' '}
                    {formatProgress(lastIssuedPerSong.get(song.id)!.progress)}
                  </span>
                )}
              </td>
            </tr>
